document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async event => {
    ...
    try {
      ...
      // If this account already has a role and it does not match
      // the role selected on the login form, show a note and stop.
      const hasExistingRole = Boolean((existingUser && existingUser.role) || confirmedRole);
      if (hasExistingRole && requestedRole && requestedRole !== normalizedRole) {
        const format = value => value.charAt(0).toUpperCase() + value.slice(1);
        const selectedLabel = format(requestedRole);
        const actualLabel = format(normalizedRole);

        await window.supabaseClient.auth.signOut();
        showAlert(
          "danger",
          `The role you selected ("${selectedLabel}") does not match your account role ("${actualLabel}"). Please choose the correct role before signing in.`
        );
        return;
      }
