document.addEventListener("DOMContentLoaded", () => {
  ...
  let qrModal = null;
  if (qrModalElement && window.bootstrap && typeof bootstrap.Modal === "function") {
...
<svg width="256" height="256" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
  ...
  async function bootstrap() {
    ...
    try {
      ...
      const badgeIds = new Set(achievements.map(item => item.badge_id).filter(id => id !== null));
      const courseIds = new Set(achievements.map(item => item.course_id).filter(id => id !== null));
      ...
      const cards = achievements.map(record => {
  ...
  document.addEventListener("click", event => {
    ...
    let payload = "ISU-Roxas DBS Badge\n";
    payload += "Student: " + studentName + "\n";
    payload += "Course: " + courseTitle + "\n";
    payload += "Badge: " + badgeTitle + "\n";
    payload += "Issued: " + issued + "\n";
    payload += "Published by: " + publisherName + "\n";
    payload += "Ref: " + detail.record.id;

    const apiUrl = "https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=" + encodeURIComponent(payload);
