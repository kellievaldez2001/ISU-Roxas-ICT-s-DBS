(function () {
  const defaultConfig = { baseUrl: "api/", mode: "mock", storageKey: "dbs_mock_store_v1" };
  const config = Object.assign({}, defaultConfig, window.apiConfig || {});

  const storageKey = typeof config.storageKey === "string" && config.storageKey.trim() ? config.storageKey : "dbs_mock_store_v1";
  const resourceMap = {
    "students.php": "students",
    "subjects.php": "subjects",
    "badges.php": "badges",
    "certificates.php": "issued_badges",
    "users.php": "users",
    "courses.php": "courses",
    "lessons.php": "lessons",
  ...
  function applyFilters(resource, collection, params) {
    ...
    return collection.filter(item => {
      return entries.every(([key, rawValue]) => {
        ...
        if (key === "student_id" && resource === "issued_badges") {
          return String(item.student_id || "") === String(value);
        }
  ...
  async function supabaseRequest(endpoint, options = {}) {
    ...
    if (upperMethod === "PUT" || upperMethod === "PATCH") {
      const targetId = resolveId(params, body);
      if (targetId === null) {
        throw new Error("Missing record id.");
      }
      const mutationBody = Object.assign({}, body);
      delete mutationBody.id;
      const { data, error } = await client.from(table).update(mutationBody).eq("id", targetId).select();
      if (error) throw error;
      if (!data || !data.length) {
  ...
  async function apiRequest(endpoint, options = {}) {
    ...
    if (config.mode === "auto") {
      try {
        return await supabaseRequest(endpoint, options);
      } catch (error) {
        console.warn("Supabase request failed. Falling back to mock storage.", error);
